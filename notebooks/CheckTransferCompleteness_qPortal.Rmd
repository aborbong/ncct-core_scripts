...
# Print summary message for duplicated files
  if(length(duplicated_files)>0){
    ...
    message("Duplicates files are written to: ", log_dir,file.path(log_dir, paste0(timestamp,"_checkup_transfer_", transfer_date)),"_duplicates.txt")
  ...
  output_table <- data.frame(
    "File Name" = c(duplicated_files, missing_files),
    "Status" = c(rep("Duplicated", length(duplicated_files)), rep("Missing", length(missing_files)))
  )
  write.table(output_table, file = file.path(log_dir, paste0(timestamp,"_checkup_transfer_", transfer_date,"_output.txt")), sep = "\t", row.names = FALSE)
...
setwd("/Users/mustermann/Documents/NCCT/Git_ncct/ncct-core_scripts/notebooks/")

transfer_date = "20251027"

qportal_report_filepath = "../test_data/251112_NCCT_VOOLSTRA_Q2181_Raw_Data.tsv"
...
```{r}
#Test dataset- Transfer 16S standard - Duplicates
#demultiplexed_files_filepath = "Z:/2307-Voolstra/ITS_16s/Batch7/16S/16S_standard/251027_results-demultiplex-16Sstandard/M45K6/NA/demultiplexed_files.txt"
# Change working directory to the notebook location
#setwd("/Users/mustermann/Documents/NCCT/Git_ncct/ncct-core_scripts/notebooks/")
...
checkup_transfer(qportal_report_filepath,demultiplexed_files_filepath,transfer_date, log_dir = "test_output/")
