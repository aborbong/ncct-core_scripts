...
checkup_transfer = function(qportal_report_filepath, 
  ...
  if(is.null(demultiplexed_files_filepath)) {
    demultiplexed_files = data.frame(ncct_filename = list.files(ncct_input_folder,pattern = paste0("^", qbic_barcode_prefix, ".*\\.(fastq|fq)\\.gz$")))
    print(head(demultiplexed_files))
  }
  else {
...
```{r}
# Test adding arguments for qbic barcode prefix and ncct input folder
# Change working directory to the notebook location
#setwd("/Users/mustermann/Documents/NCCT/Git_ncct/ncct-core_scripts/notebooks/")
